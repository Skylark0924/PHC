...
def process_motion(key_names, key_name_to_pkls, cfg):
    ...
    num_augment_joint = len(cfg.robot.extend_config) if hasattr(cfg.robot, "extend_config") else 0
    ...
    shape_new, scale = joblib.load(f"phc/data/{cfg.robot.humanoid_type}/shape_optimized_v1.pkl") # TODO: run fit_smple_shape to get this
...
def main(cfg : DictConfig) -> None:
    ...
    key_name_to_pkls = {"0-" + "_".join(data_path.split("/")[split_len+1:]).replace(".npz", ""): data_path for data_path in all_pkls}
    ...
    if len(all_data) == 1:
        ...
        dumped_file = f"phc/data/{cfg.robot.humanoid_type}/v1/singles/{data_key}.pkl"
        print(dumped_file)
