...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        if "type" in tree.getroot().find("worldbody").findall('.//joint')[0].attrib and \
                tree.getroot().find("worldbody").findall('.//joint')[0].attrib['type'] == "free":
        ...
        if hasattr(cfg, "extend_config"):
            for extend_config in cfg.extend_config:
                self.body_names_augment += [extend_config.joint_name]
                self._parents = torch.cat(
                    [self._parents, torch.tensor([self.body_names.index(extend_config.parent_name)]).to(device)], dim=0)
                self._offsets = torch.cat([self._offsets, torch.tensor([[extend_config.pos]]).to(device)], dim=1)
                self._local_rotation = torch.cat([self._local_rotation, torch.tensor([[extend_config.rot]]).to(device)],
                                                 dim=1)
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full=False, dt=1 / 30):
        ...
        if hasattr(self.cfg, "extend_config"):
        ...
        if return_full:
            rigidbody_linear_velocity = self._compute_velocity(wbody_pos,
                                                               dt)  # Isaac gym is [x, y, z, w]. All the previous functions are [w, x, y, z]
            ...
            if "extend_config" in self.cfg:
                if len(self.cfg.extend_config) > 0:
                    return_dict.dof_pos = pose.sum(dim=-1)[...,
                                          1:self.num_bodies]  # you can sum it up since unitree's each joint has 1 dof. Last two are for hands. doesn't really matter.
            else:
        ...
        for i in range(J):
            if self._parents[i] == -1:
                ...
                jpos = (torch.matmul(rotations_world[self._parents[i]][:, :, 0],
                                     expanded_offsets[:, :, i, :, None]).squeeze(-1) + positions_world[
                            self._parents[i]])
